...
#### 返回数据（LEN = 17）
数据区 16 字节（D127~D0, big-endian）+ 末尾指令号：
```
Rev(D127~D65) | POL(D64) | TX_EN_ROW(D63~D48) | 0xFFFF(D47~D32)
| FREQ(D31~D24) | Tx1_BeamV(D23~D12) | Tx1_BeamH(D11~D0) | 0x5F
```

| 字段 | 说明 |
|---|---|
| POL | 极化：0=LHCP，1=RHCP |
| TX_EN_ROW | 阵列使能 16bit：0xFFFF=开 / 0x0000=关 |
| FREQ | 频段号，实际频率 = 27500 + 50×FREQ (MHz) |
| Tx1_BeamV / Tx1_BeamH | 12bit 补码波控值（同 4.1 波束设置） |
| 0x5F | 指令号 |
...
| V2.2 | 增加查询指令2：波束参数查询 | 2026-06-12 |
