...
- 仅调整 `panel.py` 的 Qt 布局与显示密度；不修改协议字段、命令构帧、Driver、扫描计算或连接生命周期。
- 移除与设备型号下拉重复的独立标题行；串口栏独占一行。控制区改为两行两列：第一行“频点/极化”和
  “波束配置”，第二行将“衰减、阵列使能、外参/上报”横向紧凑组合，并与扫描参数并列。
- 扫描参数保持 θ、φ 各一整行；频点来源和手动 TX/RX 频点按同一行列网格排列，按钮、进度和当前状态
  保持单行，避免因标签换行造成行高不一致。
- `0x30` 状态表由“字段/值 × 2”扩为“字段/值 × 3”，23 个字段按列优先填入 8 行；固定行高并关闭
  垂直滚动条。日志只保留紧凑显示高度，把面板主数据优先留给命令和状态。
- 验收以 1920×1080 离屏窗口为基准：全部 7 类命令、扫描输入/按钮和 0x30 全字段均无需滚动即可见；
  小于该宽度时由工作区横向最小宽度保护，不裁切控件。
