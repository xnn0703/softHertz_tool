...
from soft_hertz_tool.identity import SETTINGS_APPLICATION, SETTINGS_ORGANIZATION
...
def test_async_logger_uses_current_product_log_directory(tmp_path: Path):
    logger = AsyncFrameLogger(documents_dir=tmp_path)
    assert logger.log_dir == tmp_path / SETTINGS_ORGANIZATION / SETTINGS_APPLICATION / "logs"
    logger.close()
