...
源码运行和发布产物统一使用名称 **SoftHertz_AFDTR_Tool**。本 README 描述当前 PySide6 代码线；`softHertz_upper/` 保留合并前的 PyQt5 代码线，用于追溯研发演进，不作为当前运行、打包或新增设备的实现入口，也不得直接混用其控制器、协议偏移或文档结论。
...
├── DOC/                                # 受控设备协议及 PDF 原件
├── softHertz_upper/                    # 历史 PyQt5 代码线，仅用于研发追溯
└── KauDC004A_TestTool/
