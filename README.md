...
| AFDR1024 | 波束设置；阵列使能；极化设置；状态、波束参数与校准结果查询 |
...
AFDR1024 的“查询全部状态”按 ID 依次发送 `0x9C`、`0x9F`、`0x9E`，校准结果可在状态区展开查看。温度偏移按原始值减 80 还原，其余校准项显示原始码。旧固件未回复时校准值保持 `N/A`；AFDT1024 待 TX 源码确认后接入。此项不改变客户流量模拟的查询组合。详见 [校准查询协议说明](docs/protocols/readable-notes/afdr1024-alignment-query.md)。

### AFDT1024 / AFDR1024 客户流量模拟

两个阵列面板均提供“客户流量模拟”：默认定址非等待波束周期 10 ms、查询周期 1000 ms、
运行 1 min、每条查询超时 100 ms。波束采用开始时页面频率/角度快照，不自动使能阵列或 PA。
波束与查询共用目标 ID（默认 0x01）；AFDT 使用 0x5C/0x5F，AFDR 使用 0x9C/0x9F。

- 仅用于电脑作为唯一请求方的两线 RS485 链路，连接具有自动方向切换能力的转换器，停止其他主控发送。
- 默认“定址应答＋零间隙，不等待回复”：周期波束不等回显，每次查询与一个完整定址波束合成两帧一次 write；回复只参与后台统计，不阻塞后续发送。
- 选两项查询时，连续两个波束时刻各发送一个“波束＋查询”包，不等待第一条查询回复。
- 保留“指定 ID，等待回显”：波束收到完整原帧回显或超时后才继续，查询也逐条等待回复。
- 定址等待模式零间隙表示等待结束后不额外延迟，不能合并需应答的波束与查询；指定/随机间隔从等待结束计算。
- 保留“广播，无应答”：零间隙将“广播波束＋首条查询”一次写出，另可选指定/随机间隔。
- 仅传统等待模式以匹配回复或超时控制继续发送；客户非等待模式的超时仅更新统计。
- 恢复时跳过错过周期，不补发积压波束；运行中冻结本串口手工配置。
- 停止后点击“打开导出目录”，获取自动生成的 JSONL 明细与 JSON 汇总。
  文件位于默认日志目录的 `traffic/<run_id>/`，明细按 50 MiB 轮转，不自动删除。
- 记录实际 write 起止/返回字节数、周期偏差、查询前间隔、回复延迟、超时、异常与跳过次数；波束与查询应答分开统计。
  收到帧按波束/状态/波束参数指令分类（含重复、迟到或未匹配帧），合法匹配数另列；保留 RX/DROP 原文。
  合并写入的帧内线路间隔记未测；write 返回不代表最后停止位已上线路。
- 协议没有事务序号，同目标同指令的旧回复恰好落入下一等待窗口时无法严格区分；原帧回显也无法仅凭报文排除转换器本地回环。非等待模式使用有界 FIFO 关联同类未超时请求，匹配数不是严格事务确认。

主机测试与伪终端闭环已验证，Windows 原生 EXE、真实两线 RS485 和实测线路间隔仍待验收。
详细范围及证据见 [客户流量开发记录](docs/development/afdtr-customer-traffic/development.md)。

### AFD01_QS 工作区

- 支持 QS V1.7 的 `0x01`～`0x0A` 控制命令、`0x0B/0xA1` 阵列档位查询与设置，以及 `0xA0` 实时状态解析；
- 解析 `0xA0` 实时状态，串口链路可接收约 100 Hz 数据，业务 UI 最多按 10 Hz 刷新；
- 使用滑动窗口显示 A0 上报频率，超过 1 秒未收到 A0 时显示超时；
- 通过 `0x0B/0xA1` 查询和设置 TX/RX 档位 1～5，对应 `8×8`、`10×10`、`12×12`、`14×14`、`16×16` 有效子阵；
- `0xA1` 载荷只含当前 `tx_level, rx_level` 两个字节，不解释结果、电源或应用状态；
- TX/RX 使用 16×16 客户子阵网格，区分当前、待确认和通信失败状态；
...
- **波束扫描**：可分别配置 θ 离轴角（0~90°）与 φ 方位角（0~360°）的起止、步进和间隔（1~60000 ms），频点源支持未超时的 STATUS_REPORT 当前 RF 或独立手动 TX/RX 输入；按公式 `u_x = 180 * (f / f0) * sinθ * cosφ`、`u_y = 180 * (f / f0) * sinθ * sinφ`（`f0` TX=30000、RX=20270）转换为与 KA256 V2 一致的 12 bit 回绕码，再走 `0x14 SET_BEAM` 下发；提供"开始 / 暂停 / 结束"控制。
...
模拟器支持配置回显、查询 1、查询 2、多子阵状态及 RX 校准结果查询。上位机的 AFDT1024、AFDR1024 面板分别连接两组虚拟串口的另一端。
