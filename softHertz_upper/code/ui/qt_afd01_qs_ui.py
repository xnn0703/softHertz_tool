...
class QAFD01_QS_UI(QBaseUI):
    ...
    def create_specific_widgets(self):
        ...
        self.report_power_status_off_rb.toggled.connect(lambda: self.report_power_status_var.__setattr__("value", 0))
        self.report_power_status_on_rb = QRadioButton("打开")
        self.report_power_status_on_rb.toggled.connect(lambda: self.report_power_status_var.__setattr__("value", 1))
        ...
        self.report_broadcast_lock_no_rb.toggled.connect(lambda: self.report_broadcast_lock_var.__setattr__("value", 0))
        self.report_broadcast_lock_yes_rb = QRadioButton("已锁定")
        self.report_broadcast_lock_yes_rb.toggled.connect(lambda: self.report_broadcast_lock_var.__setattr__("value", 1))
        report_layout.addWidget(self.report_broadcast_lock_no_rb, 1, 1, 1, 1)
        report_layout.addWidget(self.report_broadcast_lock_yes_rb, 1, 2, 1, 1)
        ...
        self.report_power_save_no_rb.toggled.connect(lambda: self.report_power_save_var.__setattr__("value", 0))
        self.report_power_save_yes_rb = QRadioButton("支持")
        self.report_power_save_yes_rb.toggled.connect(lambda: self.report_power_save_var.__setattr__("value", 1))
        report_layout.addWidget(self.report_power_save_no_rb, 1, 4, 1, 1)
        report_layout.addWidget(self.report_power_save_yes_rb, 1, 5, 1, 1)
        ...
        self.report_reboot_no_rb.toggled.connect(lambda: self.report_reboot_var.__setattr__("value", 0))
        self.report_reboot_yes_rb = QRadioButton("重启")
        self.report_reboot_yes_rb.toggled.connect(lambda: self.report_reboot_var.__setattr__("value", 1))
        report_layout.addWidget(self.report_reboot_no_rb, 2, 1, 1, 1)
        report_layout.addWidget(self.report_reboot_yes_rb, 2, 2, 1, 1)
        ...
        self.polarization_left_rb.toggled.connect(lambda: self.polarization_var.__setattr__("value", "左旋"))
        self.polarization_right_rb = QRadioButton("右旋")
        self.polarization_right_rb.toggled.connect(lambda: self.polarization_var.__setattr__("value", "右旋"))
        ...
        self.tracking_mode_manual_rb.toggled.connect(lambda: self.tracking_mode_var.__setattr__("value", 1))
        self.tracking_mode_auto_rb = QRadioButton("自动")
        self.tracking_mode_auto_rb.setChecked(True)
        self.tracking_mode_auto_rb.toggled.connect(lambda: self.tracking_mode_var.__setattr__("value", 0))
        ...
        self.tx_enable_off_rb.toggled.connect(lambda: self.tx_enable_var.__setattr__("value", 0))
        self.tx_enable_on_rb = QRadioButton("开启")
        self.tx_enable_on_rb.toggled.connect(lambda: self.tx_enable_var.__setattr__("value", 1))
        ...
        self.test_type_combo.currentTextChanged.connect(lambda: self.test_type_var.__setattr__("value", self.test_type_combo.currentText()))
    ...
    def _run_performance_test(self, interval, count):
        ...
        try:
            ...
            for i in range(count):
                ...
                # 发送命令，不记录中间结果
                self.device.send_command(test_type, params)
        ...
        if self.device:
            ...
            for label_text, info_key in status_mapping.items():
                if label_text in self.status_labels and info_key in device_info:
                    self.status_labels[label_text].setText(str(device_info[info_key]))
